...
function animationLoop() {
  const isBeltMoving = isRunning && activeItems.length > 0;

  if (isBeltMoving) {
    conveyorBelt.classList.remove('paused');
    conveyorBadge.textContent = `Đang chạy (${activeItems.length} phôi)`;
    ...
    if (isRunning) {
      conveyorBadge.textContent = "Chờ phôi (Đứng yên)";
      conveyorBadge.className = "badge";
      conveyorBadge.style.background = "rgba(59, 130, 246, 0.2)";
      conveyorBadge.style.color = "#60a5fa";
    }
  }
